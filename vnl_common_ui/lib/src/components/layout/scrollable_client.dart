...
class ScrollableClient extends StatelessWidget {
  ...
  Widget _buildViewport(BuildContext context, ViewportOffset verticalOffset, ViewportOffset horizontalOffset) {
  ...
  Widget build(BuildContext context) {
    ...
    final bool effectivePrimary =
        primary ?? mainAxisDetails.controller == null && PrimaryScrollController.shouldInherit(context, mainAxis);
...
class RenderScrollableClientViewport extends RenderTwoDimensionalViewport {
  ...
  void layoutChildSequence() {
    ...
    verticalOffset.applyContentDimensions(
      0,
      (child.size.height - viewportDimension.height).clamp(0.0, double.infinity),
    );
...
class _ScrollableClientChildVicinity extends ChildVicinity {
  ...
  const _ScrollableClientChildVicinity({required this.viewportSize, required super.xIndex, required super.yIndex});
}
